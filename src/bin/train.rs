...
use rust_rl::{
    agents::q_agent::QAgent, environment::move_to_center::{Board, GridEnvironment, MoveAction}, train, Agent
};
...
fn main() {
    ...
    assert_eq!(<QAgent as Agent<GridEnvironment>>::predict(&agent, &Board{ position: (2,1)}), MoveAction::Up);
